-- 누락된 하트 시스템 테이블 생성
-- 실행 방법: mysql -u root -p lovlechat < create-missing-tables.sql
...
-- 3. 기존 personas 테이블에 외래키 추가 (실패해도 무시)
ALTER TABLE personas ADD CONSTRAINT fk_personas_userId 
FOREIGN KEY (userId) REFERENCES users(userId) ON DELETE CASCADE;
...
-- 완료 메시지
SELECT '✅ 하트 시스템 테이블 생성 완료!' as result; 